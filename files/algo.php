...
for ($I = 0; $I < $subjects_count * 5; $I++) {
    ...
    for ($j = 0; $j < 40; $j++) {
        ...
        if ($j % 8 == 0)
            $r++;
        if (isset($subjectslots[$sem][$r % 5][$j % 8])) {
            //check if subjectslot is empty
            continue;
        } else if (isset($teachers[$tindex]->days[$sem % 2][$r % 5][$j % 8])) {
            ...
            // set subject
            $subjects[$i]->classes++;
            $subjectslots[$sem][$r % 5][$j % 8] = $subjects[$i]->code;
            $aliasslots[$sem][$r % 5][$j % 8][0] = $subjects[$i]->alias;
            $teachers[$tindex]->days[$sem % 2][$r % 5][$j % 8] = $subjects[$i]->code;
            $teachers[$tindex]->classroom_names[$sem % 2][$r % 5][$j % 8] = $classroom;
            break;
        }
    }
}
...
for ($i = 1; $i < 9; $i += 2) {
    for ($k = 0; $k < 5; $k++) {
        for ($j = 0; $j < 8; $j++) {
            ...
            if (isset($subjectslots[$i][$k][$j % 8])) {
            } else {
                $subjectslots[$i][$k][$j % 8] = "-";
                $aliasslots[$i][$k][$j % 8][0] = "-";
            }
...
for ($i = 1; $i < 9; $i += 2) {
    ...
    echo $database_name;
    for ($j = 0; $j < 5; $j++) {
        // echo  $subjectslots[$i][$j][0]."hello<br>";
        // echo  $aliasslots[$i][$j][1][0]."hello<br>";
        // echo  $aliasslots[$i][$j][2][0]."<br>";
        // echo  $aliasslots[$i][$j][3][0]."<br>";
        // echo  $aliasslots[$i][$j][4][0]."<br>";
        // echo  $aliasslots[$i][$j][5][0]."<br>";
        // echo  $aliasslots[$i][$j][6][0]."<br>";
        // echo  $aliasslots[$i][$j][6][1] . ", " 
        // . $aliasslots[$i][$j][6][2]."<br>"
        // . $subjectslots[$i][$j][0] . "<br>" . $aliasslots[$i][$j][0][0] . "',
        ...
        //  . $aliasslots[$i][$j][7][1] . ", " . $aliasslots[$i][$j][7][2] ;
...
period7='" . $teachers[$i]->days[1][$j][6] . "<br>" . $teachers[$i]->classroom_names[1][$j][6] . "',
period8='" . $teachers[$i]->days[1][$j][7] . "<br>" . $teachers[$i]->classroom_names[1][$j][7] . "'
 WHERE day='" . $days[$j] . "' ";
        $q = mysqli_query(mysqli_connect("localhost", "root", "", "ttms"), $query);
    }
}
